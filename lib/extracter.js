...
function findAndSetExactPositionOfArrays(xml, descriptor) {
  ...
  for (var _objName in descriptor) {
    ...
    if (_obj.type === 'array' && _obj.iterators.length > 0) {
      ...
      if (!(_pivot === null || _pivot === undefined)) {
        ...
        if (!(_realArrayPosition === null || _realArrayPosition === undefined)) {
          // TODO test if the repetition is found !!!!!!!!!!!!
          descriptor[_objName].position = {
            start: _realArrayPosition.startEven,
            end: _realArrayPosition.endEven, /* 'startOdd':_realArrayPosition.startOdd,*/
            endOdd: _realArrayPosition.endOdd
          };
          descriptor[_objName].xmlParts.push({obj: _objName, array: 'start', pos: _realArrayPosition.startEven});
          descriptor[_objName].xmlParts.push({
            obj: _objName,
            array: 'end',
            pos: _realArrayPosition.endEven /* ,'next':_realArrayPosition.endOdd*/
          });
